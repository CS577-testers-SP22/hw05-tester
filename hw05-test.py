...
def test8_timeout_tests():
    if not TIMEOUT_TEST:
        ...
        return
    print('Running Timeout Tests. These can take a long time if your code is not efficient. Set TIMEOUT_TEST=False on line 39 if you don\'t want to test this.')
    tests = dict()
    big = (2**15)
    iterations = 1000
    big_str = '\n'.join(map(str, range(big - 1, -1, -1)))
    tests['timeout-test-0'] = {'input':f'{iterations}\n' + (str(big) + '\n' + big_str + '\n')*iterations, 'output':f'{int(big*(big-1)/2)}\n'*iterations}
    big_str = '\n'.join(map(str, range(0, big)))
    tests['timeout-test-1'] = {'input':f'{iterations}\n' + (str(big) + '\n' + big_str + '\n')*iterations, 'output':f'0\n'*iterations}
    run_tests(tests)
...
def get_versions():
    ...
    try:
        with urlopen('https://raw.githubusercontent.com/CS577-testers-SP22/hw05-tester/master/.version') as f:
            if f.status != 200:
                raise Exception
...
if __name__ == '__main__':
    print(f'Running CS577 SP22 HW05 tester v{__version__}')
    ...
    if current < latest:
        ...
        print('You can download the latest version at https://github.com/CS577-testers-SP22/hw05-tester\n')
    print()
    main()
    print()
